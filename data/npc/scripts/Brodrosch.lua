...
local travelNode = keywordHandler:addKeyword({'farmine'}, StdModule.say, {npcHandler = npcHandler, onlyFocus = true, text = 'Do you seek a ride to Farmine for free?'})
travelNode:addChildKeyword({'yes'}, StdModule.travel, {npcHandler = npcHandler, premium = true, level = 0, cost =0, destination = {x=33025, y=31553, z=10} })
...
function creatureSayCallback(cid, type, msg)
	...
	if (msgcontains(msg, "cormaya") and getPlayerStorageValue(cid,storage) == 3) then
   		npcHandler:say("Do you seek a ride to Cormaya for free?",cid)
		Topic[cid] = 1
